...
[Link to the Github repository](https://github.com/abt5572/hw-5)
...
> Based on the summary , we can see that the coefficients for each
> variable represent the expected change in the median house value for a
> one-unit increase in the corresponding predictor variable, while
> holding all other variables constant. The intercept of -2.273e+06
> represents the expected median house value when all predictor
> variables are equal to zero. The adjusted R-squared value of 0.6428
> indicates that the model explains about 64.28% of the variance in the
> median house value, after adjusting for the number of predictor
> variables in the model.

Here, it is possible to appreciate that most of the covariates p-values are significant (rule of thumb), with the exception of ocean_proximity ("Near Ocean", and "Near bay") which is less significant. We have an intercept of -2.27e^6 representing the expected median house value when all varaibles are 0. And we also have a 0.64 variance in the median house value.
...
```{R}
test_mhv <- df_test$median_house_value #Creating this value for future tasks

rpart_RMSE <- rmse(test_mhv, rpart_predictions)
rpart_RMSE
```
...
# Calculating RMSE using SVM
svm_RMSE <- rmse(test_mhv, svm_predictions)
svm_RMSE
```
...
summaryTable <- data.frame(
  Model = c('Linear Regression', 'Decison Tree', 'SVM', 'Newral Network'),
  RMSE = c(lm_RMSE, rpart_RMSE, svm_RMSE, nn_RMSE)
)
summaryTable
```

Here, it is possible to visualize that the model that performed the best is the SVM model, and on second place I would put the Neural Net Model. The main reason SVM model would be the best suit for predicting this time, is because this model can handle very good non-linear data through different kernels that are used. And its predictions are usually very accurate going accordingly to the flow tendencies of the data.
...
glm_test <- predict(glm_fit, df_test, output = "response")
...
```{R}
rpart_classes <- ifelse(predict(rpart_classes, newdata = df_test) > 0.5, 1, 0)
...
```{R}
M1 <- model.matrix(spam ~ 0 + ., data = df_train) # Defining new M
...
nnet_fit <- NNet %>% 
  ...
  set_hparams(p = ncol(M1), q1 = 32, q2 = 16, q3 = 8) %>%
  set_opt_hparams(lr = 0.001) %>%
  fit(
    data = list(
...
As a person who checks his emails almost everyday, my main concern between false positives and false negatives, would be false negatives. I personally consider that it would be much better to have in your inbox an email that is a spam than having in your junk/spam folder an email that is very important and shoul have been in your inbox. In this sense, the main concern would be to be able to eliminate false negatives. And the model that would make a good work with this would be the Neural Network model for identifying spam emails, we can come to this conclusion by subtracting 1 minus the total true positive rate of the model and we find that NN is the best model for this.
...
fit_1 <- NN1 %>% 
  ...
  set_hparams(p = ncol(df_test), q1 = 10, o = 3) %>%
  set_opt_hparams(lr=0.001) %>%
  fit(
    data = list(
    ...
    epochs = 50,
    dataloader_options = list(batch_size = 150, shuffle = TRUE),
    verbose = FALSE
  )
```
...
```{R}
plot_decision_boundary(NN1) #Supposed to go fit_1_precictions
```
...
fit_0 <- NN0 %>% 
  ...
  set_hparams(p = ncol(df_test), o = 3) %>%
  set_opt_hparams(lr=0.001) %>%
  fit(
    data = list(
      df %>% select(x1, x2) %>% as.matrix,
...
```{R}
plot_decision_boundary(NN0) #Supposed to go fit_0_precictions
```
...
```{R}
# It's supposed to be called NN3 mecause of 3 hidden layers
NN3 <- nn_module(
...
fit_3 <- NN3 %>% 
  ...
  set_hparams(p = ncol(df_test), q1 = 10, q2 = 10, o = 3) %>%
  set_opt_hparams(lr = 0.001) %>%
  fit(
    data = list(
      df %>% select(x1, x2) %>% as.matrix,
...
```{R}
plot_decision_boundary(NN3) #Supposed to go fit_3_precictions
```
...
Even though I wasn't able to display correctly the decision boundary for each Nerual Network, It would believe that the NN with better model would be the one that has 3 hidden layers since when the number of hidden layers increase, then the neural network turns to represent better non-linear decision boundaries, this because Neural Networks can allow it to learn more complex and abstract representations of the input data, which can potentially lead to better performance on a given task. However, it is important to note increasing the number of hidden layers can also make the network more difficult to train and more prone to overfitting.
